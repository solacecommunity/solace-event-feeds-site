...
const PublishEvents = (props) => {
  ...
  const { isAnyEventRunning, setIsAnyEventRunning } = useContext(SessionContext); // Use context
  ...
  const toggleControl = (item) => {
    setActiveFeedConfig((prev) => (prev === item.eventName ? null : item.eventName));
  };
  ...
  useEffect(() => {
    ...
    setIsAnyEventRunning(Object.values(activeEvents).some(event => event.active));
  ...
  const startFeed = (item) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      const intervalId = setInterval(() => {
        ...
        console.log(`Publishing event ${item.eventName}: ${activeEvents[item.eventName].countSend}`);
  ...
  const getMaxDelay = (activeEvents) => {
    return Math.max(...Object.values(activeEvents).map(event => event.delay || MAX_DELAY));
  };
  ...
  return (
    <div>
      <Collapsible 
        ...
        open={!disableForm}
      >
        <div style={{ marginBottom: '16px', display: 'flex', flexDirection: 'row', gap: '8px', paddingLeft: '10px'}}>
          <Button color="danger" variant="filled" shape="round" onClick={handleDisconnect} disabled={!isConnected || isAnyEventRunning}> Disconnect Broker </Button>
        </div>
          {errorConnection && (
        ...
        {/* List of events  */}
          <div id="scrollableDiv" style={{height: 400,overflow: 'auto',padding: '0 16px'}}>
            <div style={{ marginBottom: '16px', display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', gap: '8px', padding: '0 30px'}}>
              <Button color="primary" variant="outlined" onClick={startAllFeed} disabled={!isConnected || !Object.values(activeEvents).some(event => !event.active)} >Start All</Button>
              <Button color="danger" variant="outlined" onClick={stopAllFeed} disabled={!isConnected || !isAnyEventRunning} >Stop All</Button>
            </div>
            <InfiniteScroll dataLength={feedRules.length} scrollableTarget="scrollableDiv" style={{ display: 'flex', flexDirection: 'column-reverse' }}>
              <List
                ...
                renderItem={(item) => (
                  <List.Item 
                    key={item.eventName}
                    actions={[
                      <Button color="primary" variant="filled" shape="round" onClick={() => startFeed(item)} disabled={!isConnected || activeEvents[item.eventName].active}> Start </Button>,
                      <Button color="danger" variant="filled" shape="round" onClick={() => stopFeed(item)} disabled={ !activeEvents[item.eventName].active}> Stop </Button>,
                      <Button 
                        style={{ color: activeFeedConfig === item.eventName ? '#00ad93' : 'black', background: 'none', border: 'none' }} 
                        variant="link" 
                    ...
                    <List.Item.Meta 
                      avatar={
                        <div style={{ padding: '10px 0 0 0'}}>
                          <LinkOutlined style={{ fontSize: '15px', color: activeEvents[item.eventName].timeoutId ? '#00ad93' : 'black' }} />
                        </div>
                      }  
                    ...
                    {activeFeedConfig === item.eventName && (
                      <div style={{ marginTop: '16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span>Message rate ({activeEvents[item.eventName]?.rate} msg/s)</span>
                          <Slider 
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
                          <span>Delay ({activeEvents[item.eventName]?.delay} s)</span>
                          <Slider 
                            style={{ width: '250px', flex: '1'}}  
                            defaultValue={0} 
                            min={0} 
